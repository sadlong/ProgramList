...
bool ResetDialog::checkUserValid()
{
    if(ui->user_edit->text() == ""){
        AddTipErr(TipErr::TIP_USER_ERR, tr("username is empty"));
        return false;
    }
...
void ResetDialog::initHandlers()
{
    ...
    _handlers.insert(ReqId::ID_GET_VARIFY_CODE, [this](QJsonObject jsonObj){
        ...
        showTip(tr("varify code send to email, please check"), true);
        qDebug()<< "email is " << email ;
    });
...
void ResetDialog::on_sure_btn_clicked()
{
    ...
    if(!valid){
        qDebug() << "checkUserValid error";
        return;
    }

    valid = checkEmailValid();
    if(!valid){
        qDebug() << "checkEmailValid error";
        return;
    }

    valid = checkPassValid();
    if(!valid){
        qDebug() << "checkPassValid error";
        return;
    }

    valid = checkVarifyValid();
    if(!valid){
        qDebug() << "checkVarifyValid error";
        return;
    }
    ...
    showTip("modify password successfully!", true);
}
